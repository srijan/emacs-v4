...
- A heading marked `COMMENT` does not tangle. Grep hits under one (Evil, mu4e, EAF, old org-gtd versions, Tabspaces) are dead config; the live outline is `grep -n '^\*\{1,2\} ' config.org` minus the `COMMENT` entries. Evil is off.

    ...
    (with-eval-after-load 'notmuch
      ...
      (defun my/gmail-search-url (message-id &optional email)
        "Build a Gmail search URL for MESSAGE-ID.
           Optional EMAIL specifies which Gmail account."
        (let ((base (if email
                        (format "https://mail.google.com/mail/?authuser=%s" email)
                      "https://mail.google.com/mail/")))
          (concat base "#search/rfc822msgid:" message-id)))


      (defun my/mindwtr-capture-template-with-link ()
        "Like `mindwtr-capture-template' but appends both the notmuch link (%a)
          and a Gmail search link for the same message."
        (let ((gmail-link
               (when (derived-mode-p 'notmuch-show-mode)
                 (let* ((msgid (replace-regexp-in-string
                                "^id:" ""
                                (notmuch-show-get-message-id)))
                        (email (or (notmuch-show-get-header :Delivered-To)
                                   (notmuch-show-get-header :To)))
                        (email (when email
                                 (if (string-match "<\\([^>]+\\)>" email)
                                     (match-string 1 email)
                                   (string-trim email))))
                        (url (my/gmail-search-url msgid email)))
                   (format "[[%s][Gmail]]" url)))))
          (concat (mindwtr-capture-template)
                  "%a"
                  (when gmail-link (concat " | " gmail-link))
                  "\n%i\n"))))

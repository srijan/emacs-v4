...
#+begin_src emacs-lisp :tangle "post-init.el"
  ...
  (use-package org
    ...
    ;; Session.app (macOS focus timer) integration + cross-device state sync.
    ;; On org-clock in/out we (a) drive the macOS Session timer via its session://
    ;; URL scheme, and (b) write the current session to a Syncthing-synced JSON
    ;; file so an Android home-screen widget can show the active task.  The
    ;; session:// calls are a no-op off macOS; the state file is written everywhere.
    (defun my/session-dispatch (url)
      "Open a session:// URL via macOS `open' (no-op off macOS).
  ...
  Errors are swallowed so a sync/IO hiccup never breaks clocking."
    ...
    (defun my/session-clock-in ()
      (let ((task-name (org-get-heading t t t t))
            (category (or (org-entry-get nil "MW_AREA" t) "general")))
        (my/state-write
        ...
        ;;          (url-encode-url category)))
        ))

    (defun my/session-clock-out ()
      (my/state-write '(:active :false))
      ;; (my/session-dispatch "session:///finish")
      )

    (defun my/session-clock-cancel ()
      (my/state-write '(:active :false))
      ;; (my/session-dispatch "session:///abandon")
      )

    (add-hook 'org-clock-in-hook #'my/session-clock-in)
    (add-hook 'org-clock-out-hook #'my/session-clock-out)
    (add-hook 'org-clock-cancel-hook #'my/session-clock-cancel)

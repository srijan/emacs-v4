  ...
  (use-package dirvish
    ...
    (dirvish-quick-access-entries ; It's a custom option, `setq' won't work
     '(("h" "~/"                          "Home")
       ...
       ("n" "~/Sync/docs/notes/"   "Notes")
       ("o" "~/Sync/docs/org/"     "GTD Org")
       ("c" "~/.emacs.d/"                 "Config")))
    :config
...
#+begin_src emacs-lisp :tangle "post-init.el"
  ...
  chars are safe, and a destination of 0 so the clock hook never blocks."
    ...
    (defvar my/state-file "~/Sync/docs/org/state.json"
...
#+begin_src emacs-lisp :tangle "post-init.el"
  (use-package howm
    ...
    :init
    (require 'howm-org)
    (setopt howm-directory "~/Sync/docs/notes")
...
#+begin_src emacs-lisp
  (setq my-notes-dir           (expand-file-name     "~/Sync/docs/notes/"))
...
#+begin_src emacs-lisp :tangle "post-init.el" :noweb yes
  <<denote_config>>
#+end_src
#+begin_src emacs-lisp :tangle "~/Sync/docs/notes/denote_config.el" :epilogue (format-time-string ";; Last generated on %c")
  ;;; post-init.el --- -*- no-byte-compile: t; lexical-binding: t; -*-
  ;; This file has been generated from my emacs's config.org file and is for
  ;; reference purposes only. DO NOT EDIT.
#+end_src
#+begin_src emacs-lisp :tangle "~/Sync/docs/notes/denote_config.el" :noweb yes
  <<denote_config>>
#+end_src
...
#+begin_src emacs-lisp
  ...
  (setq my-koreader-export-dir  (expand-file-name "~/Sync/Documents/koreader/clipboard/"))
  ...
  to control output format and behavior."
    ...
    (unless skip-sync
      ...
      (let* ((scp-command "scp -r -P 2222 root@192.0.2.10:/mnt/us/koreader/clipboard/ ~/Sync/Documents/koreader/")
  ...
  (use-package notmuch
    ...
    (setopt notmuch-saved-searches
            '((:name "Inbox All"    :query "tag:inbox"                        :key "i" :sort-order newest-first)
              (:name "Inbox GO"     :query "tag:inbox path:acme/**"     :key "g" :sort-order newest-first)
    ...
    (setopt message-sendmail-extra-arguments
            '("send" "--quiet" "-t" "-C" "/Users/srijan.c/Maildir/acme"))
                ...
                (lambda ()
                  ...
                  (set-process-sentinel
                   (start-process "gmi-sync" "*gmi-sync*" "/bin/sh" "-c"
                                  "gmi sync -sv -C ~/Maildir/acme && gmi sync -sv -C ~/Maildir/srijan && notmuch new")
                   (lambda (proc _event)
...
#+begin_src emacs-lisp :tangle "post-init.el"
  (use-package jira
    ...
    :demand t
    :config
    (setq jira-base-url "https://work.atlassian.example")
    (setq jira-username "you@work.example")
...
#+begin_src emacs-lisp :tangle "post-init.el"
  ...
  (use-package elfeed-protocol
    :custom
    (elfeed-feeds '(("fever+https://srijan@miniflux.example.net"
                     :api-url "https://miniflux.example.net/fever/"
                     :password (lambda ()
                                 (auth-source-pick-first-password :host "miniflux.example.net" :user "fever")))))
...
#+begin_src emacs-lisp :tangle "post-init.el"
  (use-package clatter
    ...
    (clatter-networks
     '(
       ("libera"
        ;; The TLS cert is issued for irc.example.net only (no SAN for the

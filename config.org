...
#+begin_src emacs-lisp :tangle "post-init.el"
  (use-package clatter
    ...
    ;; Clatter has no `soju.im/bouncer-networks' support, so each upstream
    ;; network needs its own connection, with the network named in the SASL
    ;; username as <user>/<network>.  Clatter hardcodes the SASL authcid to
    ;; the nick, which cannot carry the suffix (a `/' is not a legal nick,
    ;; and the nick is also sent as a literal NICK to the upstream).  Route
    ;; the authcid through `:username' instead, which clatter otherwise only
    ;; uses for the USER command.  Upstream lacks a `:sasl-username'.
    (define-advice clatter-cap--sasl-plain-authenticate
        (:override (conn) sj/sasl-username)
      "Use `:username' as the SASL authcid when set, else fall back to nick."
      (let* ((config (process-get (clatter-connection-process conn)
                                  :clatter-config))
             (authcid (or (plist-get config :username)
    ...
    (clatter-networks
     `(
       ("libera"
        ...
        :port 16502
        :tls t
        ;; Must match the nick soju already holds upstream: clatter sends a
        ;; literal NICK, which soju forwards, renaming us on the network.
        :nick "srijan"
        ;; soju selects the upstream network from the SASL authcid suffix.
        :username "srijan/libera"
        :sasl plain
        ;; Bouncer session: skip NickServ IDENTIFY and nick reclaim.
        :bouncer t
        ))))
#+end_src
** Desktop Utilities

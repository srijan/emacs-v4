...
#+end_src
** COMMENT Workspaces - Tab Bar and Bufferlo
#+begin_src emacs-lisp :tangle "post-init.el"
  ;; Tab line - show current workspace's buffers per window
  (use-package tab-line
    ...
    (setq
     ...
     (lambda ()
       (seq-filter (lambda (b) (memq b (bufferlo-buffer-list)))
                   (tab-line-tabs-fixed-window-buffers)))
     tab-line-new-button-show nil
     tab-line-close-button-show nil)
    (global-tab-line-mode 1))
  ;; Tab bar as workspace switcher
  (use-package tab-bar
    :ensure nil
    :demand t
    :bind
    ...
    (defun my/tab-bar-target-tab (buf)
      "Return a tab name for BUF based on context, or nil for no routing."
        ...
        (let ((bname (buffer-name buf))
          ...
          (cond
           ...
           ((or (equal bname "*Org Agenda*")
                (and file
                     (cl-some (lambda (af)
                                (file-equal-p file (expand-file-name af)))
    ...
    ;; Auto-switch tab when visiting a buffer that belongs to another tab
    (defvar my/--switching-tab nil
      "Guard to prevent recursive tab switching.")

    (defun my/switch-to-buffer-in-tab (orig-fn buffer-or-name &rest args)
      "If BUFFER belongs to another tab, switch to that tab first."
      (if my/--switching-tab
          (apply orig-fn buffer-or-name args)
        (let ((my/--switching-tab t))
          (when-let* ((buf (if (stringp buffer-or-name)
                               (get-buffer buffer-or-name)
                             buffer-or-name))
                      ((buffer-live-p buf)))
            (if-let ((tab-name (my/tab-bar-target-tab buf)))
                (my/tab-bar-find-or-create tab-name)
              (when-let* ((tab (tab-bar-get-buffer-tab buf nil t))
                          (tab-name (alist-get 'name tab)))
                (tab-bar-switch-to-tab tab-name))))
          (apply orig-fn buffer-or-name args))))

    (advice-add 'switch-to-buffer :around #'my/switch-to-buffer-in-tab)

    ;; Route find-file to existing project tab
    (defun my/find-file-in-project-tab (orig-fn filename &rest args)
      "If FILENAME belongs to a project with an existing tab, switch to it first."
      (unless my/--switching-tab
        (let ((my/--switching-tab t))
          (when-let* ((dir (file-name-directory (expand-file-name filename)))
                      (proj (project-current nil dir))
                      (tab-name (file-name-nondirectory
                                 (directory-file-name (project-root proj))))
                      ((seq-some (lambda (tab) (equal (alist-get 'name tab) tab-name))
                                 (tab-bar-tabs))))
            (tab-bar-switch-to-tab tab-name))))
  ...
  ;; Bufferlo - per-tab buffer isolation
  (use-package bufferlo
    :demand t
    :config
    (bufferlo-mode 1)
    ;; Consult source showing only current tab's buffers (default in C-x b)
    (with-eval-after-load 'consult
      (defface my/bufferlo-buffer-face
        '((t :inherit font-lock-string-face))
        "Face for tab-local buffers in consult.")
      (defvar my/bufferlo-consult-source
        `(:name    "Tab Buffers"
                   :narrow  ?t
                   :category buffer
                   :face    my/bufferlo-buffer-face
                   :history buffer-name-history
                   :state   ,#'consult--buffer-state
                   :default t
                   :items   ,(lambda () (mapcar #'buffer-name (bufferlo-buffer-list)))))
      (add-to-list 'consult-buffer-sources 'my/bufferlo-consult-source)))

#+end_src
** Workspaces - Tab Bar and Tabspaces
#+begin_src emacs-lisp :tangle "post-init.el"
  ;; Tab line - show current workspace's buffers per window
  ;; (use-package tab-line
  ;;   :ensure nil
  ...
  ;;    tab-line-tabs-function
  ;;    (lambda ()
  ;;      (seq-filter #'tabspaces--local-buffer-p
  ;;                  (tab-line-tabs-fixed-window-buffers)))
  ;;    tab-line-new-button-show nil
  ;;    tab-line-close-button-show nil)
  ;;   (global-tab-line-mode 1))
...
#+begin_src emacs-lisp :tangle "post-init.el"
  ...
  (use-package org-gtd
    ...
    :config
    (setopt org-gtd-directory "~/Sync/org/")
    ...
    (defun org-open-gtd ()
      (interactive)
      (find-file "~/Sync/org/org-gtd-tasks.org")
      )
...
#+begin_src emacs-lisp :tangle "post-init.el"
  (use-package howm
    ...
    :init
    (require 'howm-org)
    (setopt howm-directory "~/Sync/notes")
...
#+begin_src emacs-lisp
  (setq my-notes-dir           (expand-file-name     "~/Sync/notes/"))
...
#+begin_src emacs-lisp :tangle "post-init.el" :noweb yes
  <<denote_config>>
#+end_src
#+begin_src emacs-lisp :tangle "~/Sync/notes/denote_config.el" :epilogue (format-time-string ";; Last generated on %c")
  ;;; post-init.el --- -*- no-byte-compile: t; lexical-binding: t; -*-
  ;; This file has been generated from my emacs's config.org file and is for
  ;; reference purposes only. DO NOT EDIT.
#+end_src
#+begin_src emacs-lisp :tangle "~/Sync/notes/denote_config.el" :noweb yes
  <<denote_config>>
#+end_src

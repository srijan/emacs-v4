...
BOUNDARY is nil or not present (no overlap), return MESSAGES unchanged."
    ...
    ;; Upstream sends `CHATHISTORY TARGETS * *', but the IRCv3 spec requires
    ;; two real timestamps; soju rejects `*' with
    ;; FAIL CHATHISTORY/INVALID_PARAMS ("Invalid first bound"), so offline-DM
    ;; discovery fails on every connect.  Send an epoch..now window instead.
    (define-advice clatter-chathistory-fetch-targets
        (:override (conn &optional limit) sj/valid-targets-bounds)
      (when (clatter-chathistory--available-p conn)
        (let ((n (or limit clatter-chathistory-limit)))
          (clatter-send
           conn
           (format "CHATHISTORY TARGETS timestamp=1970-01-01T00:00:00.000Z timestamp=%s %d"

...
#+begin_src emacs-lisp :tangle "post-init.el"
  (use-package org
    ...
    ;; Skip duplicate entries in calendar main files
    ;; (defun org-entry-exists-in-other-file-p ()
    ;;   "Check if current entry exists in another agenda file.
    ;;    Compares headline text and scheduled/deadline dates."
    ;;   (let* ((current-file (buffer-file-name))
    ;;          (headline (org-get-heading t t t t))
    ;;          (scheduled (org-get-scheduled-time (point)))
    ;;          (deadline (org-get-deadline-time (point)))
    ;;          (other-files (remove current-file org-agenda-files)))
    ;;
    ;;     (catch 'found
    ...
    ;;             (org-map-entries
    ;;              (lambda ()
    ;;                (when (and (string= headline (org-get-heading t t t t))
    ;;                           (equal scheduled (org-get-scheduled-time (point)))
    ;;                           (equal deadline (org-get-deadline-time (point))))
    ;;                  (throw 'found t)))
    ;;              nil 'file))))
    ;;       nil)))

    ;; (defun org-agenda-skip-duplicates-from-calendar ()
    ;;   "Skip function to ignore entries from calendar.org if they exist elsewhere."
    ;;   (when (and (buffer-file-name)
    ;;              (string-match "calendar\\.org$" (buffer-file-name))
    ;;              (org-entry-exists-in-other-file-p))
    ;;     (org-end-of-subtree t)))

    ;;(setq org-agenda-skip-function 'org-agenda-skip-duplicates-from-calendar)
...
#+begin_src emacs-lisp :tangle "post-init.el"
  ...
  (defun my/complete-calendar-task ()
    ...
    (interactive)
    (let* ((calendar-file (expand-file-name "calendar.org" org-gtd-directory))
           (inbox-file (expand-file-name "inbox.org" org-gtd-directory))
...
#+begin_src emacs-lisp :tangle "post-init.el"
  ...
  (use-package denote-journal
    :after denote
    :custom
    (denote-journal-directory (expand-file-name "~/Sync/notes/journals/"))
...
#+begin_src emacs-lisp :tangle "post-init.el"
  ...
  (use-package elfeed-protocol
    :custom
    (elfeed-protocol-feeds '(("fever+https://srijan@miniflux.irc.example.net"
                              :api-url "https://miniflux.irc.example.net/fever/"
                              :password (lambda ()
                                          (auth-source-pick-first-password :host "miniflux.irc.example.net" :user "fever")))))
    (elfeed-protocol-enabled-protocols '(fever))
    ;; (auth-source-search :host "miniflux.irc.example.net" :port "443" :user "emacs")
  ...
  ;; (elfeed-protocol-fever-get-update-mark "fever+https://srijan@miniflux.irc.example.net" 'update)

  ;; (let* ((proto-id "fever+https://srijan@miniflux.irc.example.net")

...
BOUNDARY is nil or not present (no overlap), return MESSAGES unchanged."
    ...
    (defvar my/clatter-chathistory-max-age-days 3
      "Cap, in days, on how far back the on-join gap fetch reaches.")
    ...
    (define-advice clatter-chathistory--on-join
      ...
      (when (and clatter-chathistory-enabled
        ...
        (let* ((network (clatter-connection-network-id conn))
               ...
               ;; Persisted per-channel cursor (SECS . MSGID) -- survives a full
               ;; stop/restart, unlike the in-memory cursor.
               (hw (gethash (cons network (downcase channel)) my/clatter--log-hw))
               (hw-secs (car hw))
               (hw-msgid (cdr hw))
               ;; Floor is `my/clatter-chathistory-max-age-days' back -- NOT a
               ;; hardcoded 24h; change that var to widen/narrow the window.
               (floor-secs (float-time
                            (time-subtract (current-time)
                                           (days-to-time my/clatter-chathistory-max-age-days))))
               ;; Resume from the newest cursor we have (in-memory OR persisted),
               ;; but never reach past the floor: a short absence fetches just
               ;; the gap; a long one caps at the floor.
               (since-secs (max (or mem-secs 0) (or hw-secs 0) floor-secs))
               (since (seconds-to-time since-secs)))
          ;; Seed the render-dedup boundary with the persisted msgid so the
          ;; restart re-fetch (a timestamp bound re-includes the boundary
          ;; second, since soju compares at whole-second granularity) drops
          ;; everything up to and including the last-seen message.  `hw-secs'
          ;; still floors the fetch window above.
          (when hw-msgid
            (puthash (cons network (downcase channel)) hw-msgid
